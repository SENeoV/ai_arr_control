...
class RadarrService:
    """Client wrapper for Radarr HTTP API endpoints used by the agents."""
    ...
    async def update_indexer(self, indexer: dict) -> Any:
        """Update indexer configuration (PUT).

        Expects the full indexer payload as returned by `get_indexers()`.
        """
